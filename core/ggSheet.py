import gspread
from google.auth import exceptions
from google.oauth2.service_account import Credentials
from datetime import datetime
import os

def update_points_by_email(spreadsheet_id="1U6o6jwqR0xqNi1QYDDCJvl9s-BeKxemMRWYuFzEFx8U", 
                           email='', 
                           new_points=0):
    # Use the private key and service account email from your environment variables
    GOOGLE_PRIVATE_KEY = "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
    GOOGLE_SERVICE_ACCOUNT_EMAIL = "nodepayreporttool@test-nodepay-report.iam.gserviceaccount.com"

    # The scope needed to interact with Google Sheets
    scope = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive"]
    
    # Create a credentials object
    creds = Credentials.from_service_account_info(
        {
            "type": "service_account",
            "private_key": GOOGLE_PRIVATE_KEY,
            "client_email": GOOGLE_SERVICE_ACCOUNT_EMAIL,
            "auth_uri": "https://accounts.google.com/o/oauth2/auth",
            "token_uri": "https://oauth2.googleapis.com/token",
            "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs"
        },
        scopes=scope,
    )

    try:
        # Authenticate with Google Sheets API
        client = gspread.authorize(creds)
        sheet = client.open_by_key(spreadsheet_id).worksheet("Dawn")

        # Get the headers (first row)
        headers = sheet.row_values(1)
        
        # Get current date for 'last update' field
        current_date = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
        
        # Ensure the required headers are present
        if 'last update' not in headers or 'last update point' not in headers or 'Point total' not in headers:
            print("Error: One or more required columns are missing in the sheet.")
            return
        
        # Get column indices based on headers
        email_col = headers.index('Email') + 1
        last_update_col = headers.index('last update') + 1
        last_update_point_col = headers.index('last update point') + 1
        point_total_col = headers.index('Point total') + 1
        
        # Get all rows in the sheet
        # Manually define the expected headers to handle duplicates
        expected_headers = ['Email', 'last update', 'last update point', 'Point total']
        
        # Get all rows in the sheet, passing the expected_headers to resolve duplicates
        rows = sheet.get_all_records(expected_headers=expected_headers)
        
        # Iterate through the rows and find the matching email
        for i, row in enumerate(rows, start=2):  # Starting from row 2 (after header)
            if row['Email'] == email:
                # Get the existing points (Point total before the update)
                existing_points = row['Point total'] if isinstance(row['Point total'], (int, float)) else 0
                                
                # Update the fields
                sheet.update_cell(i, last_update_col, current_date)  # 'Last update' column
                sheet.update_cell(i, last_update_point_col, existing_points)  # 'Last update point' column
                sheet.update_cell(i, point_total_col, new_points)  # 'Point total' column
                return

    except exceptions.GoogleAuthError as error:
        print(f"Authentication failed: {error}")

# Usage
# spreadsheet_id = "1U6o6jwqR0xqNi1QYDDCJvl9s-BeKxemMRWYuFzEFx8U"  # Replace with your actual Google Sheet ID
# email_to_update = "bull4001@veer.vn"  # Replace with the email to update
# new_points = 15000  # New points to add

# update_points_by_email(spreadsheet_id, email_to_update, new_points)
